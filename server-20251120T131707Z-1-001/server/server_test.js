...
app.get("/api/timetable", async (req, res) => {
  try {
    const timetable = await dbAll("SELECT * FROM timetable ORDER BY startTime ASC");
    res.json(timetable);
  } catch (err) {
...
app.post("/api/timetable", async (req, res) => {
  ...
  try {
    await dbRun("INSERT INTO timetable (subject, startTime, endTime) VALUES (?, ?, ?)", [subject, startTime, endTime]);
...
app.put("/api/timetable/:id", async (req, res) => {
  ...
  try {
    await dbRun(
      "UPDATE timetable SET subject = ?, startTime = ?, endTime = ? WHERE id = ?",
      [subject, startTime, endTime, id]
    );
